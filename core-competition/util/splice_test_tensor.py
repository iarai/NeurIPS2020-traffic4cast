...
if __name__ == '__main__':
    ...
    try:
        ...
        print('usage: extract_channel.py -i <input h5 file> - c <first channel> -c <second channel> ... -o <output_file>')
        sys.exit(2)

    for opt, arg in opts:
        if opt in "-h":
            print('usage: splice_test_tensor.py -i <input h5 file> - c <first channel> -c <second channel> ... -o <output_file>')
            sys.exit(2)
        if opt in ("-c","--channel"):
    ...
    util.write_data_to_h5(out_array[0], output_file)
    print("File writen.")
